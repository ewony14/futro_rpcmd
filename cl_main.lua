RegisterNetEvent("futro_rpcmd:event")
local players = {}
local doPlayers = {}
            ...
            if NetworkIsPlayerActive(p) then
                ...
                if DoesEntityExist(ped) then
                    ...
                    if not ons then
                        x = (x*100)
                        y = (y*100)
                        str = str.."<p style=\"left: "..x.."%;top: "..y.."%;-webkit-transform: translate(-50%, 0%);max-width: 100%;position: fixed;text-align: center;color: #ffffff;border-radius:3px;\"><b>⠀"..b.message.."⠀</b></p>"
                    end
                end
            end
        end
            ...
            if NetworkIsPlayerActive(p) then
                ...
                if DoesEntityExist(ped) then
                    ...
                    if not ons then
                        x = (x*100)
                        y = (y*100)
                        str = str.."<p style=\"left: "..x.."%;top: "..y.."%;-webkit-transform: translate(-50%, 0%);max-width: 100%;position: fixed;text-align: center;color: #0059FF;border-radius:3px;\"><b>⠀"..b.message.."⠀</b></p>"
                    end
                end
            end
        end
...
AddEventHandler("futro_rpcmd:event", function(source, message, type1)
    ...
    if #(GetEntityCoords(GetPlayerPed(monid)) - GetEntityCoords(GetPlayerPed(sonid))) < 8.0 and HasEntityClearLosToEntity( GetPlayerPed(monid), GetPlayerPed(sonid), 17 ) then
        if type1 == 0 then 
            players[source] = {message = fix(message), rtime = 2500+GetGameTimer()} 
        elseif type1 == 1 then 
            doPlayers[source] = {message = fix(message), rtime = 2500+GetGameTimer()} 
        end
    end
end)
